...
class backpack:
    ...
    def solve(self):
        for i in range(1, 2 ** self.item_count):
            ...
            self.weight = 0  # вес взятых предметов
            ...
            for index, state in enumerate(items_set):
                ...
                if state == '1':
                    ...
                    cost += items[index][1]
                    self.weight += items[index][0]   # и вес
                    if self.weight > self.back_pack_size:  # если переполнен, то ставим флаг
                        overflow = True
                        break
...
items = [[1, 1], [2, 4], [3, 3], [4, 4]]

test = backpack(item_count=4, back_pack_size=5, items=items)
test.solve()
print(
